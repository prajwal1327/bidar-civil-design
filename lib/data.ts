export const companyHighlights = [
  { value: '252+', label: 'Projects Completed' },
  { value: '15+', label: 'Years Experience' },
  { value: '300+', label: 'Happy Clients' },
  { value: '12', label: 'Engineers & Architects' },
];

export const partnerLogos = [
  'https://images.unsplash.com/photo-1512290923902-8a9f81dc236c?auto=format&fit=crop&w=300&q=80',
  'https://images.unsplash.com/photo-1494526585095-c41746248156?auto=format&fit=crop&w=300&q=80',
  'https://images.unsplash.com/photo-1505693416388-ac5ce068fe85?auto=format&fit=crop&w=300&q=80',
  'https://images.unsplash.com/photo-1524758631624-e2822e304c36?auto=format&fit=crop&w=300&q=80',
  'https://images.unsplash.com/photo-1534777416015-3fa2a5d3b0e6?auto=format&fit=crop&w=300&q=80',
];

export const serviceList = [
  { title: 'Luxury Home Construction', subtitle: 'Premium villas, farmhouses and turnkey residences.' },
  { title: 'Architecture & Interior Design', subtitle: 'Sequential planning, spatial harmony and custom finishes.' },
  { title: 'Civil Engineering Consultancy', subtitle: 'Structural design, site planning and approval management.' },
  { title: 'AutoCAD Drafting & BIM', subtitle: 'DWG, shop drawings, Revit modeling and documentation.' },
  { title: 'Project Management', subtitle: 'Turnkey supervision, procurement and quality assurance.' },
  { title: '3D Visualization & Walkthroughs', subtitle: 'Photoreal renders, VR-friendly tours and concept imagery.' },
];

export const allProjects = [
  {
    title: 'Lakhsetty Hospital',
    category: 'Healthcare',
    location: 'Mangalpet',
    client: 'Dr. Rajashekhar Lakhsetty',
    image: '/assets/hospital-aerial.jpg',
    services: ['Structural Design', 'Architecture', 'Project Management'],
  },
  {
    title: 'Prayavi Hospital',
    category: 'Healthcare',
    location: 'Bidar, Karnataka',
    client: 'Dr. Amar Yarolkar',
    image: '/assets/circular-building.jpg',
    services: ['Architecture', 'Civil Engineering', 'MEP Coordination'],
  },
  {
    title: 'Prayavi Ayurvedic College & Hospital',
    category: 'Institutional',
    location: 'Bidar, Karnataka',
    client: 'Surabhi NGO',
    image: '/assets/building-aerial.jpg',
    services: ['Architecture', 'Structural Design', 'Interior Design'],
  },
  {
    title: 'Suresh Biradar Residence',
    category: 'Residential',
    location: 'Mange Layout',
    client: 'Suresh Biradar',
    image: '/assets/render-house.jpg',
    services: ['Architecture Design', 'Interior Styling', 'Project Management'],
  },
  {
    title: 'Nagesh Chamble Apartment',
    category: 'Apartment',
    location: 'Shivajinagar, Gumpa',
    client: 'Nagesh Chamble',
    image: '/assets/site-sunset.jpg',
    services: ['Structural Design', 'BOQ Preparation', 'Project Planning'],
  },
  {
    title: 'Kalyankar Apartment',
    category: 'Apartment',
    location: 'Humnabad',
    client: 'Kalyankar',
    image: '/assets/roof-structure.jpg',
    services: ['Structural Design', 'Civil Engineering', 'Project Planning'],
  },
  {
    title: 'Jyoti Kheni Commercial Building',
    category: 'Commercial',
    location: 'Bhalki',
    client: 'Jyoti Kheni',
    image: '/assets/site-work.jpg',
    services: ['Architecture', 'MEP Coordination', 'Structural Design'],
  },
  {
    title: 'Bansal Classes',
    category: 'Institutional',
    location: 'Bidar, Karnataka',
    client: 'Bansal Classes',
    image: '/assets/groundbreaking.jpg',
    services: ['Architecture', 'Structural Design', 'AutoCAD Drafting'],
  },
  {
    title: 'Shivkumar Patil Residence',
    category: 'Residential',
    location: 'Shivanagar, Bidar',
    client: 'Shivkumar Patil Sikindrapur',
    image: '/assets/foundation.jpg',
    services: ['Architecture Design', 'Interior Styling', 'Landscape Design'],
  },
  {
    title: 'Veeresh Biradar Residence',
    category: 'Residential',
    location: 'Humnabad, Bidar',
    client: 'Veeresh Biradar',
    image: '/assets/ceiling-work.jpg',
    services: ['Architecture Design', 'Structural Design', 'Project Management'],
  },
];

export const featuredProjects = allProjects.slice(0, 3);

export const testimonials = [
  {
    quote: 'Hatkar Construction delivered our hospital building at Mangalpet with exceptional structural integrity and professionalism. Every deadline was met without compromising quality.',
    name: 'Dr. Rajashekhar Lakhsetty',
    title: 'Hospital Building, Mangalpet',
  },
  {
    quote: 'The Prayavi Hospital project in Bidar was executed with great precision. Er. Pramod and his team understood our medical facility requirements perfectly and delivered beyond expectations.',
    name: 'Dr. Amar Yarolkar',
    title: 'Prayavi Hospital, Bidar',
  },
  {
    quote: 'Building the Prayavi Ayurvedic College and Hospital was a large-scale undertaking. Hatkar Construction managed the entire project with outstanding coordination and quality control.',
    name: 'Surabhi NGO',
    title: 'Prayavi Ayurvedic College & Hospital, Bidar',
  },
  {
    quote: "My residential building at Mange Layout came out exactly as I envisioned. The team's attention to detail and use of premium materials made all the difference.",
    name: 'Suresh Biradar',
    title: 'Residential Building, Mange Layout',
  },
  {
    quote: 'The apartment project at Shivajinagar, Gumpa was handled with great efficiency. From design to handover, the Hatkar team was transparent and highly responsive.',
    name: 'Nagesh Chamble',
    title: 'Apartment, Shivajinagar, Gumpa',
  },
  {
    quote: "Our apartment in Humnabad was delivered on time and within budget. Excellent workmanship and a team that truly cares about the client's satisfaction.",
    name: 'Kalyankar',
    title: 'Apartment, Humnabad',
  },
  {
    quote: 'The commercial building at Bhalki reflects the premium standard Hatkar Construction consistently delivers. Solid engineering and a beautiful finish.',
    name: 'Jyoti Kheni',
    title: 'Commercial Building, Bhalki',
  },
  {
    quote: 'Our institutional building for Bansal Classes in Bidar was built to the highest specifications. The structural quality and timely completion speak for themselves.',
    name: 'Bansal Classes',
    title: 'Institutional Building, Bidar',
  },
  {
    quote: 'I am very pleased with my residential building at Shivanagar, Bidar. Hatkar Construction brought professionalism and craftsmanship to every stage of construction.',
    name: 'Shivkumar Patil Sikindrapur',
    title: 'Residential Building, Shivanagar, Bidar',
  },
  {
    quote: 'Our home in Humnabad was built with care and precision. The team maintained great communication throughout and the final result exceeded all our expectations.',
    name: 'Veeresh Biradar',
    title: 'Residential Building, Humnabad, Bidar',
  },
];

export const blogPosts = [
  {
    title: 'How to Choose a Premium Home Design Partner',
    category: 'Design Tips',
    date: 'July 2026',
    excerpt: 'A complete guide to selecting architecture, engineering and construction partners for luxury homes.',
  },
  {
    title: 'Trend Report: Modern Villa Architecture',
    category: 'Architecture',
    date: 'June 2026',
    excerpt: 'Contemporary facades, indoor-outdoor flow and finishes that elevate living experience.',
  },
  {
    title: 'Cost Estimation Best Practices for Builders',
    category: 'Construction',
    date: 'May 2026',
    excerpt: 'How to plan budgets, material quantities and timelines for residential and commercial projects.',
  },
];
